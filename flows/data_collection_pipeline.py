...
from prefect.flows import logger
from prefect.task_runners import ThreadPoolTaskRunner

from schemas.announcement_schema import AiAnnouncementResponse, AnnouncementSchema, LLMAnnouncementResponse
...
def filter_existing_urls(url_and_last_mods: list[tuple[str, datetime]]) -> list[tuple[str, datetime]]:
    logger = logging.getLogger(__name__)
...
async def parse_content_from_url(url_and_last_mod: tuple[str, datetime]) -> AnnouncementSchema:
...
def get_ai_response(announcement: AnnouncementSchema) -> tuple[AnnouncementSchema, AiAnnouncementResponse]:
    logger = logging.getLogger(__name__)
    ...
    ai_announcement_response: LLMAnnouncementResponse = extract_position_and_summary(announcement)
...
def insert_to_db(announcement_and_ai_response: list[tuple[AnnouncementSchema, AiAnnouncementResponse]]):
    logger = logging.getLogger(__name__)
...
@flow(name="data collection pipeline", task_runner=ThreadPoolTaskRunner(max_workers=10), log_prints=True) # type: ignore
    ...
    for batch in chunked(filtered_urls, 10):
        ...
        for announcement_and_ai_response in ai_response_results:
            try:
                ...
                if res: list_to_insert.append(res)
